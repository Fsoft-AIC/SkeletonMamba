...
class MultiModalDataset(Dataset):
    def __init__(
        ...
        save_mean_std_path = f'/LOCAL2/anguyen/faic/quang/zigma/data/mean_std_mamba_{window}.npz'
        if not os.path.exists(save_mean_std_path):
            self.data_dict = []
            for data_id in self.egocentric_paths:
                print(data_id)
                ...
                query = self.process_window_data(trans, global_orient, body_pose, 0, seq_length)
                ...
                motion_input = torch.cat((global_jpos, global_rot_6d), dim=-1).reshape(-1, 24, 9) # T X (24*3+24x6)
                idx=0
                while idx + window < seq_length:
                    self.data_dict.append(motion_input[idx:idx+window])
                    idx += window
                # last window
                # window_data = motion_input[idx:]
                # if window_data.shape[0] < 120:
                #     window_data = torch.cat([window_data, torch.zeros(window-window_data.shape[0], 24, 9)], axis=0)
                # self.data_dict.append(window_data)
            
            print(f"Total number of windows for {mode}:{len(self.data_dict)}")
            
            data_stack = torch.stack(self.data_dict)
            self.mean = data_stack.mean(axis=0)
            self.std = data_stack.std(axis=0)
            print(f"mean: {self.mean}, std: {self.std}")
            np.savez(save_mean_std_path, mean=self.mean, std=self.std)
        else:
            data = np.load(save_mean_std_path)
            self.mean = data['mean']
            self.std = data['std']
    ...
    def __getitem__(self, index):
        ...
        data_input_dict['seq_len'] = actual_seq_len * 24

        return data_input_dict 

def main(opt):
    opt.canonicalize_init_head = False
    dataset = MultiModalDataset(opt, mode=opt.mode, window=opt.window)
