...
try:
    from dis_mamba.mamba_ssm.ops.triton.selective_state_update import (
...
try:
    from dis_mamba.mamba_ssm.ops.triton.layernorm import (
        RMSNorm,
        layer_norm_fn,
        rms_norm_fn,
    )
...
class Mamba(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.group_joints = kwargs.get("group_joints", None)
        self.group_joints_reverse = kwargs.get("group_joints_reverse", None)
        ...
        _s_or_t = self.st_order[self.layer_idx]
        if self.scan_type.startswith("zzvideo_") and _s_or_t == "t":
            ...
            self.in_proj = nn.Linear(
                self.d_model * 6, self.d_inner * 2, bias=bias, **factory_kwargs
            )
            self.middle_proj = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)
            self.out_proj = nn.Linear(
                self.d_inner, self.d_model * 6, bias=bias, **factory_kwargs
            )
        ...
        assert (
            (scan_type in ["v1", "v2"])
            or scan_type.startswith("zzvideo_")
        ...
        if scan_type.startswith("parallelN"):
            ...
            self.D_b._no_weight_decay = True
        elif scan_type.startswith("zzvideo_") and _s_or_t == "s":
            A_b = repeat(
                torch.arange(1, self.d_state + 1, dtype=torch.float32, device=device),
                "n -> d n",
                d=self.d_inner,
    ...
    def forward(self, hidden_states, inference_params=None):
        hidden_states = self._mamba_inner_forward(hidden_states, inference_params)
        return hidden_states

    def _mamba_inner_forward(self, hidden_states, inference_params=None):
        """
        hidden_states: (B, L, D)
        ...
        if (
            ...
            if self.scan_type == "v2":
                ...
                out_b = mamba_inner_fn_no_out_proj(
                    xz.flip(
                        [-1]
                    ),  # Flipping the xz is the same as flipping the x, as x will be fed into a "RNN"-like layer, while z will only go to a MLP layer.
                ...
                #### rearrange back done
            elif self.scan_type.startswith("zzvideo_"):
                _s_or_t = self.st_order[self.layer_idx]
                ...
                if _s_or_t == "s":
                    xz_main = rearrange(
                        xz_main, "b (t k) c -> (b t) c k", t=video_frames
                    )
                    # print('1, xz_main.shape:', xz_main.shape)
                    xz_main = xz_main[:, :, self.group_joints].contiguous()
                    # print('2, xz_main.shape:', xz_main.shape)
                    ## global mamba
                    xz_main = rearrange(
                        xz_main, "b c (m n) -> b m (c n)", n=6
                    )
                    # print('2.1, group xz_main.shape:', xz_main.shape)
                    xz = self.in_proj(xz_main).permute(0, 2, 1) # (BT), d_inner*2, 5
                    # print('2.2, after linear xz.shape:', xz.shape)
                    # print(_perm)
                    xz = xz[:, :, _perm].contiguous()  # [B,C,T]
                    # print('2.3, xz.shape:', xz.shape)
                    out = mamba_inner_fn(
                        xz,
                        self.conv1d.weight,
                        self.conv1d.bias,
                    ...
                    o_main = out[:, _ex:, :]  # here out is [B,T,C]!!!!
                    o_main = o_main[:, _perm_rev, :].contiguous()
                    o_main = rearrange(o_main, "b k (m n) -> (b k) m n", m=6)
                    # print('4, o_main.shape:', o_main.shape)
                    o_main = self.middle_proj(o_main).permute(0,2,1) # (5B, 6, 4D)
                    # print('5, o_main.shape:', o_main.shape)
                    A_b = -torch.exp(self.A_b_log.float())
                    out = mamba_inner_fn(
                        o_main,
                        self.conv1d_b.weight,
                    ...
                    o_main = rearrange(out, "(b k) m n -> b (k m) n", k=5)
                    # print('7, o_main.shape:', o_main.shape)
                    o_main = o_main[:, self.group_joints_reverse, :].contiguous()
                    # print('8, o_main.shape:', o_main.shape)
                    o_main = rearrange(o_main, "(b t) k c -> b (t k) c", t=video_frames)
                    # print('9, o_main.shape:', o_main.shape)
                elif _s_or_t == "t":
                    # inproject
                    xz = rearrange(
                            self.in_proj.weight @ rearrange(xz, "b l d -> d (b l)"),
                            "d (b l) -> b d l",
                            l=seqlen,
                        )
                    ...
                    # print('1t, xz.shape:', xz.shape)
                    # rearrange
                    xz = rearrange(
                        xz, "b c (t k) -> (b k) c t", t=video_frames
                    )
                    # print('2t, xz.shape:', xz.shape)
                    # permute
                    xz = xz[:, :, _perm].contiguous()  # [B,C,T]
                    # print('3t, xz.shape:', xz.shape)
                    out = mamba_inner_fn(
                        xz,
                        self.conv1d.weight,
                        self.conv1d.bias,
                    ...
                    o_main = out[:, _ex:, :]  # here out is [B,T,C]!!!!
                    o_main = o_main[:, _perm_rev, :].contiguous()
                    # print('4t, o_main.shape:', o_main.shape)
                    o_main = rearrange(o_main, "(b k) t c -> b (t k) c", k=wh_size)
                    # print('5t, o_main.shape:', o_main.shape)
                else:
                    raise NotImplementedError
                out = o_main
            else:
...
class Mamba_Vanilla(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.use_jit = kwargs.get("use_jit", False)
        _s_or_t = self.st_order[self.layer_idx]
        if scan_type.startswith("zzvideo_") and _s_or_t == "s":
            self.in_proj = nn.Linear(
                self.video_frames, self.d_inner * 2, bias=bias, **factory_kwargs
            )
            self.out_proj = nn.Linear(
                self.d_inner, self.video_frames, bias=bias, **factory_kwargs
            )
        else:  
            self.in_proj = nn.Linear(
                self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs
    ...
    def _mamba_inner_forward(self, hidden_states, inference_params=None):
        ...
        if (
            ...
            if self.scan_type == "v2":
                ...
                if _s_or_t == "s":
                    xz_main = xz_main.permute(0, 2, 1)
                    # print('1, xz_main.shape:', xz_main.shape)
                    xz = self.in_proj(xz_main).permute(0, 2, 1) # (BT), d_inner*2, d_model
                    # print('2.2, after linear xz.shape:', xz.shape)
                    out = mamba_inner_fn(
                        xz,
                        self.conv1d.weight,
                    ...
                    o_main = out[:, _ex:, :]  # here out is [B,T,C]!!!!
                    o_main = o_main.permute(0, 2, 1)
                    # print('3. o_main.shape:', o_main.shape)
                elif _s_or_t == "t":
                    # inproject
                    xz = rearrange(
                            self.in_proj.weight @ rearrange(xz, "b l d -> d (b l)"),
